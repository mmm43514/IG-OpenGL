...
void Textura::enviar()
{
   ...
   glGenTextures(1, &ident_textura); // idTex = nuevo identificador
   glBindTexture(GL_TEXTURE_2D, ident_textura); // cambio ident de textura activa a la de esta textura
   gluBuild2DMipmaps( GL_TEXTURE_2D, 
   GL_RGB, // formato interno
   ancho, // núm. columnas (arbitrario) (GLsizei)
   alto, // núm. filas (arbitrario) (GLsizei)
   ...
   //glLightModeli(GL_LIGHT_MODEL_COLOR_CONTROL, GL_SINGLE_COLOR); // por defecto
   glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); // por defecto
...
void Material::activar( Cauce & cauce )
{
   ...
   Tupla3f r_ambiente(k_amb, k_amb, k_amb);
   Tupla3f r_difusa(k_dif, k_dif, k_dif);
   Tupla3f r_psd_especular(k_pse, k_pse, k_pse);
   cauce.fijarParamsMIL(r_ambiente, r_difusa, r_psd_especular, exp_pse);
}
//**********************************************************************
